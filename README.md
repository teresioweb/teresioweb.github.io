...
- **Clump animation** (Un discorso di Capellaro, Quercia): the thumbnails in `.clump-stage` slide in and land clustered, on page load — pure CSS. Discorso's six photos come from different sides; Quercia's two both launch from the same off-screen point on the right (a same-side, staggered choreography chosen after one photo entering from the left turned out to be the source of a Chrome-only animation lag — see the comment above `body.page-quercia .clump-photo img` in `css/style.css`), each with its own inline `animation-duration`/`--delay` so the left-bound photo lands first and the right-bound one, entering mid-flight, lands just after. Discorso uses six photos at 370×240px each with all the page's text sitting below in a single dark translucent `.ritagli-band`; Quercia reuses the same mechanism for just two photos at 630×415px, above its own (shorter) `.ritagli-band` intro paragraph. On mobile, the clump switches to a plain vertical stack at readable size instead of the clustered layout.
