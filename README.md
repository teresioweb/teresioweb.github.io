...
Weights included: Merriweather 300/400/700/900 + 400 italic, Merriweather Sans 300/400/700 + 400 italic. Every one of these is actually used somewhere on the site *except* Merriweather 300 (serif light) — it's declared in `@font-face` but no current rule applies that weight in the serif family; harmless to leave (browsers only fetch a declared font if some rule actually requests it) but just as fine to drop if you'd rather trim the `fonts/` folder further. Merriweather Sans 500 was dropped for the same reason — it was never referenced by anything.

## 1. Images
All content photos are served as WebP, sized close to their actual display context (roughly 2× the CSS display width, for retina screens) rather than at whatever resolution the source scan or camera happened to produce — a card thumbnail shown at 180px isn't served a 2000px-wide file. Three exceptions stay as full-resolution JPG instead of WebP: `images/diploma-cfm.jpg`, `images/teresio-cavaliere.jpg`, and `images/curriculum-fronte.jpg` are each linked twice on the Curriculum page — once as a WebP thumbnail sized for on-page display, and once via the "Immagine" badge, which deliberately points to the original, undownsized JPG so "view full size" actually means full size. `images/creative.png` (the footer's Creative Commons badge) and every SVG in `sfondi/` are untouched — already small enough that neither conversion nor resizing would meaningfully help.

Four images do double duty as `og:image`/`twitter:image` social-preview sources as well as on-page thumbnails (`card-tecnigrafo`, `logos-pintori`, `curriculum-fronte`, `discorso/capellaro`) — each is sized to at least 1200px on the long edge, above the usual social-platform minimum, even where the on-page thumbnail itself would need less.
...
- **Side-by-side image + description**: any `<figure class="reveal-img side">` shows the image and its `<figcaption>` next to each other (stacks on mobile). Add `from-right` to flip which side the image is on, `align-top` to align the text to the top of the image instead of centering it vertically, and `crop-top` to crop the image from the top instead of the center. Used on the Logos 27 and Curriculum pages.
- **Image badges** (Logos 27, Curriculum): every linked image gets a small pill badge in the bottom corner (PDF / Video / Link / Immagine) via `.img-badge`, plus a subtle darkening overlay on hover, so it's clear upfront that the image is clickable and what it opens.
- **Clump animation** (Un discorso di Capellaro): the thumbnails in `.clump-stage` (370×240px each) slide in together from different sides and land clustered, on page load — pure CSS. All the page's text sits below it, inside a single dark translucent `.ritagli-band`. On mobile, the clump switches to a plain vertical stack at readable size instead of the clustered layout.
...
- **Quercia scroll windows**: the two long Facebook-post screenshots sit in fixed-height (`373px`, `280px` on mobile) `.quercia-window` containers with a native vertical scrollbar, instead of running the full image height inline. Each has a small "Ingrandisci" button that stays pinned to the top-right while scrolling (`position: sticky`) and opens the full-resolution image in a new tab (plain link, not the site's lightbox — this page has no lightbox markup at all).
...
- The lightbox is a real `<dialog>` opened with `.showModal()`, not a styled `<div>` — focus trapping, Escape-to-close, and top-layer stacking all come from the browser rather than hand-rolled JS. A small supplementary Tab handler patches one Chromium quirk (focus briefly passing through `document.body` at the wrap points instead of cycling directly between controls); everything else about the trap is native. Lightbox triggers are real `<button>` elements, so focus and Enter/Space activation are native too.
