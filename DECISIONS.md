...
| §AO | The burger without JavaScript, and four text fixes |
...
**Lightbox alt.** `overlayImg.alt` is set to `""` deliberately, and the
description reaches a screen reader through `.lightbox-caption` instead. Until
16 September 2026 this paragraph justified that by saying the caption was always
the thumbnail's `alt` or a longer version of it. On Foto it is neither: all six
`data-caption`s are short titles ("Brevetto di volo") against a full
description in the `alt`, so inside the dialog the description was never read —
not on open, and not on any photo reached with prev/next.

Two things now carry it. Where `data-caption` differs from the `alt`,
`showIndex()` appends the `alt` to the caption as a `.visually-hidden` span, so
the live region announces title and description together on every step. And
`initLightbox()` gives the dialog `aria-describedby` pointing at the caption,
because the caption is written before `showModal()`, and a live region filled
while its dialog is closed announces nothing on the first open — the same
silence the counter's comment already recorded.

Checked in Chromium's accessibility tree: dialog name "Immagine ingrandita",
description "Brevetto di volo . Teresio di fronte ad aeroplano a elica con altre
persone" (the space before the full stop is Chromium's, for an absolutely
positioned child, and is not spoken). Not tested with NVDA or VoiceOver. The
thumbnail keeps its `alt`, which names the trigger button.
...
the accessibility tree. Until 16 September 2026 the badge was part of the
button's accessible name, computed from its contents, so with JS off the name
lost "⤢ Lista". The button now carries its own `aria-label` — "Lista completa dei
brevetti" / "List of all patents", opening with the visible "Lista"/"List" for
WCAG 2.5.3 — which also stopped the drawing's `alt` and the ⤢ glyph being read
as the name of a control that opens a list. The label is the same on both paths:
with JS off it names a button whose list is printed right below it.
...
characters. **Since 16 September 2026 the label starts with the visible text**,
"EN – English version" / "IT – Versione italiana": a name that did not contain
"EN" failed WCAG 2.5.3, and a voice-control user saying "click EN" matched
nothing.
...
**A side effect not worth having, fixed 16 September 2026.** `preventDefault`
also cancels the move of the sequential focus starting point that a real
navigation makes, so focus stayed on the ornament at the foot of the page: a
keyboard user saw the top and the next Tab went to the footer mailto (WCAG
2.4.3). The handler now also calls `main.focus({ preventScroll: true })`, the
same target the skip link uses. Checked in Chromium with Enter three times in a
row: focus on `#main` each time, and the next Tab on the first link of the
essay.
...
`scrollIntoView` respects the target's `scroll-margin-top` — 100px when this was
written, 12px on top of `html`'s `scroll-padding-top` since §AH — and with no
`behavior` argument resolves to the computed `scroll-behavior` — smooth once §G
has armed it, instant under `prefers-reduced-motion` — so the function does not
need to know about either. Targets carrying `tabindex` also get focus, because
that is what a real fragment navigation does: it is how the skip link reaches
`<main tabindex="-1">`.

**Until 16 September 2026 only `<main>` carried one.** The four `#clump-*`
photos, `#banchelli` and the Home footnotes (`#nota`, `#nota-1`, `#nota-2`) did
not, so from the second use of a link the scroll arrived but focus stayed on the
link, and the next Tab went back to wherever the link was: from "(*)" that was
the hero photo, some 3000px up (WCAG 2.4.3). The first use was always fine,
because a native fragment navigation moves the sequential focus starting point
even to an element that cannot take focus. All eight targets now carry
`tabindex="-1"` in the markup, and `outline: none` in the stylesheet for §13's
reason: the jump is the feedback, not a box drawn round a paragraph or a photo.
The handler itself did not change. Checked in Chromium, three consecutive
Enters on "(*)" and on "documento": target at the same position each time,
focus on the target, next Tab on the first link inside it.
...
| Index shows four cards for five destinations | By choice — the missing card is **Foto** (`galleria.html`), which is in the nav like the other four. Quercia is not the fifth destination and never was: it is in neither the nav nor the cards, for the reason two rows above. Corrected 18 September 2026; the row had named Quercia since it was written |
| Comments shipped to readers: no minification step | Accepted. `README.md`'s first line is the constraint — no build tools — and the served files are the edited files. §AI keeps the cost in range by pruning the source instead, twice now; the figure to watch is the gzipped one, 30,302 bytes of CSS and 21,001 of JS at gzip level 9, measured 16 September 2026 — not the raw. **Measured with Python's `gzip.compress(data, 9)`, and say so**: GNU `gzip -9 -n` returns 30,111 and 21,001 for the same two files, so the JS agrees, the CSS is 191 lower, and the disagreement looks exactly like a stale figure until you reach for the other tool (§AN). The gap between the two tools is not a constant — it was 167 bytes before the 15 September comment edits and 178 after them — so it cannot be carried forward; re-measure both. Bytes, and stated as bytes: see the unit note in §AI's opening section |
...
| Entries no code comment points to | Seven of the 124 entries: §N, §Q, §U, §W, §Z, §AG, §AN. (§AI, §AK and §AL are containers whose subentries are pointed at, so they reach. §AN is pointed at from `README.md`, which is not what this row counts.) Content is right in each, the pointer is missing — measured 14 September 2026, when this row still said §Z was the only one of 122. §F left the list on 16 September 2026, when `showIndex()` gained a pointer to it. **Entries here, not headings.** The file has 138 headings, the extra fourteen being the subentries §AI.1 to §AL.2, and this row read "seven of 137" until 15 September 2026 — which made its own denominator jump 122 → 137 while a single entry was added, and hid that the basis had changed rather than the file. The row above counts headings, correctly, because a `§` reference can point at a subentry; this one counts entries, because only a whole entry can go unpointed. **Say which you are counting whenever both numbers are in range** |
...
### §AO.1 — Below 67.5em, no JavaScript meant no navigation

The README promised that without `animations.js` the content still shows. The
content did; the way to the rest of the site did not. Below the nav breakpoint
every destination and the language link live in `.nav-links`, which is
`visibility: hidden` until `initMobileMenu()` adds `.open` — and with JS off, or
with `animations.js` in 404, nothing adds it. The burger took the tap and did
nothing. From Home, Curriculum, Foto and the other language were unreachable;
measured at 375px on both no-JS paths (disabled outright, and script served as
404).

The fix is CSS only, inside the `max-width: 67.5em` block, because a stylesheet
rule is the one thing both no-JS paths share (§O explains why the inline
`<head>` script covers only one of them):

- `.menu-toggle` is hidden, so there is no control that does nothing.
- `.site-nav` becomes `position: relative`, and `body` drops its
  `padding-top` reserve and `html` its `scroll-padding-top`. A fixed bar that
  suddenly holds three wrapped rows of links would cover the `<h1>`; in the
  flow it pushes the page down instead. The bar's hide-on-scroll is a JS
  behaviour anyway, so nothing is lost.
- `.nav-links` is reset from the dropdown to a static, wrapped row under the
  brand, `flex: 1 1 100%` so it takes its own line in `.nav-inner`.

Verified at 320, 390 and 1000px on both no-JS paths: all six links visible and
inside the viewport, the `<h1>` below the bar. With JavaScript on, at the same
widths, nothing changed: `.nav-links` closed, bar height and `<h1>` position as
before. Above the breakpoint the rules do not apply.

### §AO.2 — Text

- **English dates are British throughout.** Two US-format dates had survived
  among the day-month-year ones, against `og:locale` `en_GB`: `en/home.html`
  (31 December 2018) and `en/discorso.html` (1 January 1960).
- **"Spilla d'0ro" is "Spilla d'Oro"** in Discorso's transcript of Banchelli's
  first post and in the matching `.visually-hidden` span. The zero is a typing
  slip in the original post, not a reading; a screen reader announced it as
  "zero". This is the one deliberate departure from the letter of that
  transcript.
- **Quercia: [*sic*] after "Novembre"**, IT and EN. Massimo's quoted message
  dates the fall to 18 December 2015 and, two sentences later, to November. The
  quotation stays verbatim; the marker says the contradiction is the source's.
- **`lang="fr"` on Logos 27** now covers the whole French title, *Ranimer une
  belle mécanique, la super-calculatrice Logos 27-2*, on the Italian page as it
  already did on the English one, and is added to *L'oeil numérique* on both.
