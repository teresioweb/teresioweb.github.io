...
function initMobileMenu() {
  ...
  const setOpen = (open) => {
    ...
    // aria-expanded is the only state the button reports; its aria-label
    // stays "Menu". Swapping the label too (Apri/Chiudi) made the state be
    // announced twice.
...
function initLightbox() {
  ...
  if (overlayCaption) {
    if (!overlayCaption.id) overlayCaption.id = "lightbox-caption";
    overlay.setAttribute("aria-describedby", overlayCaption.id);
  }
  ...
  const showIndex = (index) => {
    ...
    // alt="" on the enlarged <img>: the description reaches a screen
    // reader through the caption below instead, which is an aria-live
    // region (announced on prev/next) and the dialog's aria-describedby
    // (read on open, when a live region set before showModal() is
    // silent). Where data-caption differs from the thumbnail's alt — all
    // six on Foto, where the caption is a short title — the alt is
    // appended to the caption as a .visually-hidden span, so the
    // description is announced once and never lost. → DECISIONS.md §F
    overlayImg.alt = "";
    if (overlayCounter) {
    ...
    if (overlayCaption) {
      ...
      if (alt && alt !== caption) {
        ...
        const sep = /[.!?]\s*$/.test(overlayCaption.textContent) ? " " : ". ";
        desc.textContent = sep + alt;
        overlayCaption.appendChild(desc);
      }
    }
  };
...
// nothing.
//
// scrollIntoView lands the target under html's scroll-padding-top plus its
// own 12px scroll-margin-top (§AH) and, with no `behavior` given, resolves
// to the computed scroll-behavior — smooth once §G has armed it, instant
// under prefers-reduced-motion. The target then gets focus, as a real
// fragment navigation would give it: without that, a script-driven scroll
// leaves focus — and the next Tab — on the link the reader started from.
// Every target on the site carries tabindex="-1" for this (<main>, the
// four #clump-* photos, #banchelli, the Home footnotes); one without it
// would scroll but keep focus behind.
...
// preventDefault also cancels the focus move a real navigation would make,
// so focus goes to <main> by hand: otherwise it stays on the ornament at the
// foot of the page and the next Tab lands in the footer.
